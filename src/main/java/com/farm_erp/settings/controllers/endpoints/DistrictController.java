...
import com.farm_erp.settings.controllers.services.DistrictService;
...
@Path("business/settings/district")
@Produces("application/json")
@Consumes("application/json")
@Tag(name = "Settings (District)", description = "Manage  district")
...
public class DistrictController {
    ...
    @POST
    @Transactional
    @Operation(summary = "Save district", description = "This will create new  district")
    ...
    public Response create(DistrictRequest request, @Context SecurityContext ctx) {
        Principal caller = ctx.getUserPrincipal();
        String user = caller == null ? "anonymous" : caller.getName();

        _SessionStatus session = SessionValidator.validate(user, CategoryEnum.SETTINGS, CategoryItemEnum.DISTRICTS,
                Privilege.CREATE);
        if (session.status) {
    ...
    @Transactional
    @Path("/{id}")
    @Operation(summary = "Update the  district", description = "This will update the  district.")
    @APIResponse(description = "Success", responseCode = "200", content = @Content(schema = @Schema(implementation = District.class)))
    @APIResponse(description = "Invalid  setting selected", responseCode = "404")
    public Response update(@PathParam("id") Long id, DistrictRequest request,
    ...
    @GET
    @Transactional
    @Operation(summary = "Get  district", description = "This will get the  district.")
    ...
    @Transactional
    @Path("/{id}")
    @Operation(summary = "Delete the  district", description = "This will delete the  district.")
    @APIResponse(description = "Success", responseCode = "200", content = @Content(schema = @Schema(implementation = District.class)))
    @APIResponse(description = "Invalid  setting selected", responseCode = "404")
